...
void Camera::_update_view_mat() { 
	_view_mat = glm::lookAt(_position, _position + _forward, _up); 
	_inverse_view_mat = glm::inverse(_view_mat);
}

void Camera::_update_projection_mat() 
{
	_projection_mat = glm::perspectiveFov(_vert_fov, (float) _width, (float) _height, _z_near, _z_far);
	_inverse_projection_mat = glm::inverse(_projection_mat);
}
...
void Camera::move(glm::vec3 direction, bool fasterMovement, float deltaTime) 
{
	...
	if (fasterMovement) direction *= 4.0f;
...
void Camera::set_direction(glm::vec3 forward, glm::vec3 up)
{
	_forward = glm::normalize(forward);
	_right = glm::normalize(glm::cross(_forward, up));
	_up = glm::normalize(glm::cross(_right, _forward));
	
	// Note: note checked for rotation constraints
	_yaw = atan2f(_forward.z, _forward.x);
...
const glm::mat4& Camera::get_inverse_view_mat() const { return _inverse_view_mat; } 
...
const glm::mat4& Camera::get_inverse_projection_mat() const{ return _inverse_projection_mat; }

float Camera::get_yaw() const { return _yaw; };
float Camera::get_pitch() const { return _pitch; }

float Camera::get_horizontal_scale() const { return _projection_mat[0][0]; };